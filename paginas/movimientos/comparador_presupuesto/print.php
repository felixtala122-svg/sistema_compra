...
$detalle = $db->conectar()->prepare(
    "SELECT dc.id_detalle_comparador, dc.cantidad, p.nombre_producto, p.precio
     FROM detalle_comparador dc
     LEFT JOIN productos p ON dc.id_productos = p.id_productos
     WHERE dc.comparador_presupuesto = :id"
);
...
<head>
    <meta charset="utf-8" />
    <title>Comparador #<?= htmlspecialchars($id) ?></title>
    <link rel="stylesheet" href="/assets/vendor/bootstrap/css/bootstrap.min.css">
</head>
<body>
<div class="container">
    <h3>Comparador de Presupuesto #<?= htmlspecialchars($id) ?></h3>
    <table class="table">
        <thead>
            <tr>
                <th>Producto</th>
